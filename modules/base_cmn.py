...
class BaseCMN(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn):
        ...
        if self.args.useVTFCM:
            tcam = TextToImageCrossAlignModule(mem_dim=98, multi_atten=c(attn), d_model=self.d_model,topk=32, 
                                            in_linear_feature=[self.d_model, self.d_model], out_linear_feature=[self.d_model, self.d_model], maxLenReport=self.args.max_seq_length)
        else:
            tcam = None
        # todo：修改
        model = Transformer(
    ...
    def __init__(self, args, tokenizer):
        ...
        if args.useVTFCM:
            self.visualMatrix = nn.Parameter(torch.normal(mean=1,std=0.05,size=(98, self.d_model)), requires_grad=False).cuda()
